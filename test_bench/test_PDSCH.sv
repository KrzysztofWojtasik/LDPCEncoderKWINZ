...
//
// Cel:
// - Test modułu PDSCHTx, który pracuje na strumieniu 64-bit (chunk) i generuje
//   strumień wyjściowy z informacją o paddingu po lewej/prawej stronie chunku.
// - out_last pojawia się TYLKO RAZ: na końcu całego TB.
//
// Dane testowe (Matlab) – JEDEN katalog VEC_DIR_MAT:
// - <CASE>_in_bits.txt   : wejściowy strumień bitów (może być rozbity na wiele linii)
// - <CASE>_out_bits.txt  : golden bitstream (może być rozbity na wiele linii; tokeny "-1" ignorowane)
// - <CASE>_meta.txt      : meta w formacie KEY=VALUE (m.in. A jako TB length)
//
// Przebieg:
// - Wejście: tb_cb[0] zawiera cały TB (zebrany z pliku wejściowego).
// - Driver pakuje bity do 64-bit słów MSB-first i podaje do DUT.
// - Wyjście: monitor wycina z out_chunk tylko WAŻNE bity według out_pad_left/out_pad_right,
//   zapisuje je do pliku oraz zbiera do kolejki dut_bits.
// - Porównanie: dut_bits porównywane z golden_flat (golden złączony z wielu linii).
//
// Obsługa błędów:
// - Błędy są raportowane i zliczane; symulacja nie jest przerywana.
// - Test kończy się na out_last (DUT) albo na TIMEOUT.
// ============================================================================
...
module test_PDSCH;
  ...
  localparam string VEC_DIR_MAT    = "../vectors/PDSCH/Output_Matlab";
  localparam string VEC_DIR_RESULT = "../vectors/PDSCH/Output_DUT";
  ...
  localparam bit DBG_EN = 1'b0;   // 1=włącz, 0=wyłącz
  ...
  // Wejście jako jeden strumień TB w tb_cb[0]
  bit tb_cb[int][$];
  bit golden_cb[int][$];

  // META (zostawione pola + A)
  int META_CB_COUNT = 0;
  int META_K        = -1;
  int META_F        = -1;
  int META_ZC       = -1;
  ...
  initial begin
    if ($test$plusargs("DUMP")) begin
      $dumpfile("test_PDSCH.vcd");
      $dumpvars(0, test_PDSCH);
    end
  end
    ...
    if (DBG_EN) begin : gen_dbg
      always @(posedge clk) begin
        ...
        if (!rst) begin
          ...
          if (dut.out_valid_crc != prev_out_valid_crc || dut.out_last_crc !=  prev_out_last_crc ||
              dut.out_valid_prep !=  prev_out_valid_prep || dut.out_last_chunk_prep != prev_out_last_chunk_prep ||
              dut.out_last_cb_prep != prev_out_last_cb_prep) begin
            $display("STATE CHANGE @cyc=%0d: out_valid_crc=%0d, out_last_crc=%0d, out_valid_prep=%0d, out_last_chunk_prep=%0d, out_last_cb_prep=%0d, out_chunk_crc=%0b ",
              cyc, dut.out_valid_crc, dut.out_last_crc, dut.out_valid_prep, dut.out_last_chunk_prep,
              dut.out_last_cb_prep, dut.out_chunk_prep
            );
  ...
  // === POMOCNICZE: trim + detekcja bitów ===
  ...
  // GOLDEN: ignoruj KAŻDE "-1" (separator)
  ...
  // INPUT: cały TB w tb_cb[0] (zbierany z wielu linii)
  task automatic read_input_lines(
    ...
    while ($fgets(line, fd)) begin
      ...
      // bierz wszystkie 0/1 i ignoruj "-1"
  ...
  // Meta: czytaj KEY=VALUE (A używane jako tb_len)
  ...
  // Driver: wysyła TB jako strumień 64-bit (MSB-first), bez in_last_cb.
  ...
  initial begin : main
    ...
    fork
      ...
      // MONITOR: wycina ważne bity wg paddingu, zapisuje do pliku i buduje dut_bits
      begin : mon
        bit golden_flat[$];
        bit dut_bits[$];
        ...
        forever begin
          ...
          if (out_last) begin
            ...
            if (total_cb_failed == 0 && total_bit_errors == 0) begin
              $display("PASS: %s | dut_bits=%0d golden_bits=%0d", CASE, dut_bits.size(), golden_flat.size());
            end else begin
